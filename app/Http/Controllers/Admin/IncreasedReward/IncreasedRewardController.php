...
namespace App\Http\Controllers\Admin\IncreasedReward;

use App\Http\Controllers\Controller;
use App\Models\Campaign;
use App\Models\IncreasedReward;
use App\Models\Rank;
use App\Models\Affiliate;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class IncreasedRewardController extends Controller
{
  public function list(Request $request, $campaign_id)
  {
    try {
      $list = IncreasedReward::with('rank')
        ->with('campaign')
        ->with('affiliate')
  ...
  public function delete(Request $request, $id) {
    try {
      IncreasedReward::where('id', $id)->delete();
    } catch (\Exception $e) {
      return response()->json(['errors' => ['save' => $e->getMessage()]], 400);
    }
  ...
  public function bulk_edit(Request $request) {
    ...
    try {
      IncreasedReward::whereIn('id', $input['ids'])->update($data);
    } catch(\Exception $e) {
